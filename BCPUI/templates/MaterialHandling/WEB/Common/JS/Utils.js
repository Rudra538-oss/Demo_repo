...
sap.oee.ui.Utils = function () {
	return { // start Of Utils Object
		...
		oeeDateTimeFormatter: sap.ui.core.format.DateFormat.getDateTimeInstance({
			style: "medium"
		}),
		oeeTimeFormatter: sap.ui.core.format.DateFormat.getTimeInstance({
			style: "medium"
		}),
		oeeDateFormatter: sap.ui.core.format.DateFormat.getDateInstance({
			style: "medium",
		...
		fuzzySearch: function (controller, jsonModel, sValue, oBinding, sEvent, propOfModel, prevFilters, andModeOnPrevFilters) {
			...
			if (jsonModel != undefined && propOfModel != undefined && oBinding != undefined) {
				...
				var orOfNewFilter = new sap.ui.model.Filter({
					filters: newFilters,
					and: false
				});
				...
				if (prevFilters != undefined && prevFilters.length > 0) {
					...
					var orOfPrevFilter = new sap.ui.model.Filter({
						filters: prevFilters,
						and: andMode
					});
					var andOfPrevNewFilter = new sap.ui.model.Filter({
						filters: [orOfNewFilter, orOfPrevFilter],
						and: true
					});
		...
		convertContextToJSONObjects: function (aContextArray) {
			...
			for (i in aContextArray) {
				aObjs.push(aContextArray[i].getObject());
			}
		...
		buildPerformanceStrip: function (oController, performanceDetailJSON, offset) {
			...
			var performanceStrip = new sap.viz.ui5.StackedColumn({
				...
				background: new sap.viz.ui5.types.Background({
					visible: false
				}),
				dataset: dataset
			});
			...
			if (performanceDetailJSON != undefined) {
				if (performanceDetailJSON.details !== undefined) {
					...
					performanceStrip.attachInitialized(function (oEvent) {
						sap.oee.ui.Utils.fXAxisFormatter(oEvent, performanceDetailJSON, offset);
					});
		...
		removePlantTimezoneTimeOffsetAndSendUTC: function (timeInMillis, plantTimezoneOffset) {
			var timeInMillisAfterAdjustments = undefined,
				offsetInMinutes, dateTimeString;
			var date = new Date(timeInMillis);
		...
		setShiftInformationForApplication: function (shiftJSON, appData, appModel) {
			if (shiftJSON && shiftJSON.shiftDefinition) {
		...
		bindMachineList: function (payload) {
			try {
				...
				if (payload.sFragmentId && payload.controller && payload.fItemSelect) {
					...
					var oMachineModel = new sap.ui.model.json.JSONModel({
						machineList: oMachineData
					});
					...
					if (!oMachineTemplate) {
						oMachineTemplate = new sap.m.ObjectListItem({
							type: "Active",
							press: [payload.fItemSelect, payload.controller],
							markFlagged: {
								path: "bottleNeck",
								formatter: bottleNeckFormatter
							},
							showMarkers: true
						});
					...
					if (controller.noDataFound && type === sap.oee.ui.oeeConstants.configurationScreenMasterPageControlType.TREE) {
						...
						if (type === sap.oee.ui.oeeConstants.configurationScreenMasterPageControlType.TREE) {
							...
							template = new sap.m.ObjectListItem({
								type: "Active",
								press: [sItemSelect, controller]
							});
							for (var keys in properties) {
								switch (keys) {
									case "description":
										if (properties.hasOwnProperty("type") && properties.type === "order") {
											template.bindProperty("title", {
												path: properties.description,
												formatter: sap.oee.ui.Formatter.formatRemoveLeadingZero
											});
										} else {
									...
									case "secondStatus":
										if (properties.hasOwnProperty("type") && properties.type === "order") {
											listSecondStatus.bindProperty("text", {
												path: properties.secondStatus,
												formatter: sap.oee.ui.Formatter.formatDate
											});
		...
		openUser: function () {
			...
			var oPopover = new sap.m.ActionSheet({
				buttons: [new sap.m.Button({
						text: controller.oBundle.getText("OEE_BTN_CHANGE_CLIENT_AND_PLANT"),
						icon: "sap-icon://action-settings",
						press: that.onChangeClientAndPlant.bind(that, controller, sSelectClientAndPlant)
					}),
					new sap.m.Button({
						text: controller.oBundle.getText("OEE_HYPERLINK_HELP"),
						icon: "sap-icon://sys-help",
						press: that.helpLink.bind(that, sHelpLink)
					}),
					new sap.m.Button({
						text: controller.oBundle.getText("OEE_LOGOUT_BTN"),
						icon: "sap-icon://log",
						press: that.logoutUser.bind(that, controller)
					})
				]
			});
			if (controller.getView().sViewName !== "" && controller.getView().sViewName !== undefined) {
				if (controller.getView().sViewName === "sap.oee.m.configuration.ActivityConfigurations") {
					oPopover.addButton(new sap.m.Button({
						text: controller.oBundle.getText("OEE_BUTTON_COPY_ALL_ACTIVITIES"),
						icon: "sap-icon://duplicate",
						press: controller.onPressCopyActivities.bind(that, controller)
					}));
				} else if (controller.getView().sViewName === "sap.oee.m.configuration.PODConfigurations") {
					oPopover.addButton(new sap.m.Button({
						text: controller.oBundle.getText("OEE_BUTTON_COPY_ALL_PODS"),
						icon: "sap-icon://duplicate",
						press: controller.onCopyPOD.bind(that, controller)
					}));
				} else if (controller.getView().sViewName === "sap.oee.m.configuration.OrderStatusMaster") {
					oPopover.addButton(new sap.m.Button({
						text: controller.oBundle.getText("OEE_BUTTON_COPY_ALL_ORDER_STATUS"),
						icon: "sap-icon://duplicate",
						press: controller.onPressCopyOrderStatus.bind(that, controller)
					}));
				}
			}
		...
		onChangeClientAndPlant: function () {
			...
			if (!this.clientPlantDialog) {
				this.clientPlantDialog = new sap.m.Dialog({
					title: controller.oBundle.getText("OEE_LABEL_SELECT_CLIENTPLANT")
				});
				if (!dialogModel) {
					var dialogModel = new sap.ui.model.json.JSONModel();
				}
				dialogModel.setData({
					root: data.details.results
				});
				var oList = new sap.m.List({
					mode: "SingleSelectMaster",
					itemPress: that.onSelectClientAndPlant.bind(that, controller, sSelectClientAndPlant)
				});
				...
				this.clientPlantDialog.addButton(new sap.m.Button({
					text: controller.oBundle.getText("OEE_BTN_CANCEL"),
					press: that.closeclientPlantDialog.bind(that)
				}));
			}
		...
		buildPerformanceStripNew: function (oController, performanceDetailJSON, downtimeDetailModel, downtimeDetailsDialog, offset) {
			var that = this,
				currentTime, currentTimeBasedOnPlantZone;
			var available = [],
				scheduled = [],
				unscheduled = [],
				shiftBrks = [];
		...
		adjustAvailableIntervals: function (availableIntervals, eventObject) {
			var modifiedObjectAfterAdjustment = {},
				oController = this,
				newAvailableIntervals = [],
				matchedIndex, newObjectToBeAdjusted;
		...
		checkLineBehaviour: function (data) {
			var index, downtimes, indexWithMaxCount = 0,
				lengthOfObject, orderReferenceObjectIndex;
		...
		preparePerformanceData: function (lineDowns, performanceDetailJSON) {
			var intervals = [],
				intervalObject = {},
				downtimeData = [],
				data, intervalsAndDowntime, returnedLists;
			this.counter = 0;
		...
		createDowntimeData: function (data, intervals, isScheduled) {
			var index, downtimeAffectingLines = [],
				downtimeData = [],
				downtimeObject = {};
		...
		createPerformanceChart: function (data, available, scheduled, unscheduled, shiftBrks, performanceDetailJSON, downtimeDetailModel, downtimeDetailsDialog, offset) {
			var that = this,
				plantTimezoneOffsetForStartTimestamp = this.appData.plantTimezoneOffset,
				plantTimezoneOffsetForEndTimestamp = this.appData.plantTimezoneOffset;
			var offsetInMinutesForStartTimestamp, offsetInMinutesForEndTimestamp;
			/*
			 * Browser will always shows the time in browser timezone if time stamp fields are passed to chart
			...
			this.downtimeDataModel.setData({
				data: data
			});
			...
			// Adjustment to performace strip start and end scales as per plant time zone
			...
			if (this.appData) {
				...
				if (offsetInMinutesForStartTimestamp) {
			...
			if (this.appData) {
				...
				if (offsetInMinutesForEndTimestamp) {
					plantTimezoneOffsetForEndTimestamp = parseFloat(offsetInMinutesForEndTimestamp);
				}
			}

			performanceDetailJSON.endTimestamp = performanceDetailJSON.endTimestamp + (new Date().getTimezoneOffset() * 60 * 1000) + +plantTimezoneOffsetForEndTimestamp;
			performanceDetailJSON.startTimestamp = performanceDetailJSON.startTimestamp + (new Date().getTimezoneOffset() * 60 * 1000) + +plantTimezoneOffsetForStartTimestamp;
			...
			oVizFrame.attachSelectData(function (oEvent) {
				sap.oee.ui.Utils.selectDowntime(oEvent, downtimeDetailModel, downtimeDetailsDialog);
			});
			oVizFrame.attachRenderComplete(function (oEvent) {
				sap.oee.ui.Utils.fXAxisFormatterNew(oEvent, performanceDetailJSON, offset);
			});
			return oVizFrame;
		},

		adjustTimestampsToBeShownInPlantTimezone: function (dataInput) {
			var index, plantTimezoneOffsetForStartTimestamp = this.appData.plantTimezoneOffset,
				plantTimezoneOffsetForEndTimestamp = this.appData.plantTimezoneOffset;
			var offsetInMinutesForStartTimestamp, offsetInMinutesForEndTimestamp;
			for (index = 0; index < dataInput.length; index++) {
				...
				dataInput[index].startTimestamp = dataInput[index].startTimestamp + (new Date().getTimezoneOffset() * 60 * 1000) + +plantTimezoneOffsetForStartTimestamp;
				dataInput[index].endTimestamp = dataInput[index].endTimestamp + (new Date().getTimezoneOffset() * 60 * 1000) + +plantTimezoneOffsetForEndTimestamp;
			}
		},
		...
		openDowntimeDetails: function (selectedDowntime, source, downtimeDetailModel, downtimeDetailsDialog) {
			...
			if (selectedDowntime.nodeDescription) {
				details.push({
					label: this.appComponent.oBundle.getText("OEE_LABEL_WORKUNIT"),
					value: selectedDowntime.nodeDescription
				});
			}
			if (selectedDowntime.eventType) {
				details.push({
					label: this.appComponent.oBundle.getText("DOWNTIME_TYPE"),
					value: selectedDowntime.eventType
				});
			}
			if (selectedDowntime.startTimestamp && selectedDowntime.startTimestamp !== "") {
				details.push({
					label: this.appComponent.oBundle.getText("OEE_LABEL_START_TIME"),
					value: sap.oee.ui.Utils.oeeDateTimeFormatter.format(new Date(parseFloat(selectedDowntime.startTimestamp)))
				});
			}

			if (selectedDowntime.endTimestamp && selectedDowntime.endTimestamp !== "") {
				details.push({
					label: this.appComponent.oBundle.getText("OEE_LABEL_END_TIME"),
					value: sap.oee.ui.Utils.oeeDateTimeFormatter.format(new Date(parseFloat(selectedDowntime.endTimestamp)))
				});
			...
			if (selectedDowntime.eventType) {
				if (selectedDowntime.eventType === sap.oee.ui.oeeConstants.timeElementTypes.unscheduledDown) {
					if (selectedDowntime.crewSize && parseFloat(selectedDowntime.crewSize) !== 0) {
						details.push({
							...
							value: selectedDowntime.crewSize
						});
					}
				}
				if (selectedDowntime.eventType === sap.oee.ui.oeeConstants.timeElementTypes.scheduledDown) {
					if (selectedDowntime.standardDuration && selectedDowntime.standardDuration !== "0") {
						details.push({
							label: this.appComponent.oBundle.getText("OEE_LABEL_STANDARD_DURATION"),
							value: selectedDowntime.standardDuration
						});
					}
				}
				if (selectedDowntime.eventType === sap.oee.ui.oeeConstants.timeElementTypes.changeOver) {
					if (selectedDowntime.fromMaterial && selectedDowntime.fromMaterial !== "") {
						details.push({
							label: this.appComponent.oBundle.getText("OEE_LABEL_FROM_MATERIAL"),
							value: selectedDowntime.fromMaterial
						});
					}
					if (selectedDowntime.toMaterial && selectedDowntime.toMaterial !== "") {
						details.push({
							label: this.appComponent.oBundle.getText("OEE_LABEL_TO_MATERIAL"),
							value: selectedDowntime.toMaterial
						});
					}
				}
			}
			...
			if (selectedDowntime.comments && selectedDowntime.comments !== "") {
				details.push({
					...
					value: selectedDowntime.comments
				});
			}
			if (this.appData && this.appData.node && this.appData.node.lineBehavior) {
				if (this.appData.node.lineBehavior === sap.oee.ui.oeeConstants.multiplierLineBehaviourConstant) {
					if (selectedDowntime.multiplier) {
						details.push({
							label: this.appComponent.oBundle.getText("OEE_LABEL_MULTIPLIER"),
							value: selectedDowntime.multiplier
						});
					}
				}
			}
			if (selectedDowntime.changedBy && selectedDowntime.changedBy !== "") {
				details.push({
					label: this.appComponent.oBundle.getText("REPORTED_LAST_CHANGED_BY_LABEL"),
					value: selectedDowntime.changedBy
				});
			}
			if (selectedDowntime.changeTimestamp && selectedDowntime.changeTimestamp !== "") {
				details.push({
					label: this.appComponent.oBundle.getText("OEE_LABEL_DATE_TIME_LAST_REPORTED"),
					value: sap.oee.ui.Formatter.formatTimeStampWithoutLabel(selectedDowntime.changeTimestamp, this.appData.plantTimezoneOffset, this.appData.plantTimezoneKey)
				});
			}
			downtimeDetailModel.setData({
				details: details
			});
...
//////////////////////////////////////////////Sorting of lines//////////////////////////////////////////////////////
function sortinglines(Plant, Client, Language, wrkcntrdrp, Error,Flag) {

	var refresh = new Date();
	var WorkcenterModel = new sap.ui.model.xml.XMLModel();
	WorkcenterModel.loadData("/XMII/Illuminator?QueryTemplate=MaterialHandling/CustomMenu/QueryTemplates/XACQ_GetLines&Param.1=" + Plant + "&Param.2=" + Language + "&Param.4=0&Param.5=0&Param.6="+Flag+"&cache=" + refresh + "&Content-Type=text/xml", "", false);
	...
	if (oDataFlag == 1) {
		...
		$.ajax({
			...
			success: function (data) {
				...
				WorkcenterModel.loadData("/XMII/Illuminator?QueryTemplate=MaterialHandling/CustomMenu/QueryTemplates/XACQ_GetLines", "Param.1=" + Plant + "&Param.2=" + Language + "&Param.3=" + encodeURIComponent(ODataForPH) + "&Param.4=1&Param.5=0&Param.6="+Flag+"&cache=" + new Date() + "&Content-Type=text/xml", false, "POST");

			},
			error: function (errorThrown) {

                                               WorkcenterModel.loadData("/XMII/Illuminator?QueryTemplate=MaterialHandling/CustomMenu/QueryTemplates/XACQ_GetLines&Param.1=" + Plant + "&Param.2=" + Language + "&Param.4=0&Param.5=1&Param.6="+Flag+"&cache=" + refresh + "&Content-Type=text/xml", "", false);
			},
			url: eventurl
		});
	}
	var desc = WorkcenterModel.getProperty('/Rowset/Row/DESCRIPTION');
	WorkcenterModel.setSizeLimit(100000);
...
function refreshPageIRPT(){
			var url =  "/XMII/CM/MaterialHandling/CustomMenu/index.irpt?spnego=disabled" ;
...
function confirmLogOff(){
		var dialog = new sap.m.Dialog({
				...
				content: [new sap.m.Text({
						id:"textForConfirm1",
						text: getPropertyValue(oResourceModel, "LOGOFF_CONFIRMATION")
					}),
					new sap.m.Text({
						id:"textForConfirm2",
						text: getPropertyValue(oResourceModel, "LOGOFF_CONFIRM_MSG")
					})],
				beginButton: new sap.m.Button({

					text: getPropertyValue(oResourceModel, "NPDashboard_Confirm"),
					icon: "sap-icon://accept",
					press: function () {
						dialog.close();
						 $.ajax({
                                                                                type: "GET",
                                                                                url: "/webdynpro/resources/sap.com/tc~lm~itsam~ui~mainframe~wd/_wd_execute_logout?logoffurl=/",
																				success: function (data) {  
																						refreshPageIRPT();
																				},
																				error: function (xhr, status, thrownError) {
																															var error_msg = xhr.status +" "+thrownError;
																					 										sap.ui.commons.MessageBox.alert(getPropertyValue(oResourceModel, "LOGOFF_ERROR")+error_msg,sap.ui.commons.MessageBox.Icon.INFORMATION,getPropertyValue(oResourceModel, "TransferDisplay_Message"));
...
function logoff(e){
	...
	var oLogoutItem = new sap.m.Button({
			text : getPropertyValue(oResourceModel, "POPOVER_LOGOUT"),
			icon : "sap-icon://log",
			press: function () {
						oActionSheet.close();
						confirmLogOff();	
...
function getUTCCurrentDateSeconds(date) {
	...
	var mins = datenow.getUTCMinutes();
	if (mins <= 9)
		mins = ("0" + mins).slice(-2);
	...
	var Milisecs = datenow.getMilliseconds();
	

	var datetime = year + "-" + month + "-" + date + " " + hours + ":" + mins + ":" + secs +":"+Milisecs;
	return datetime;
